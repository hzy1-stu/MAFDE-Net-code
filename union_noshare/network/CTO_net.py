...
class DM(nn.Module):
    ...
    def forward(self, xr, dualattention):
        ...
        x = -1*(torch.sigmoid(crop_3)) + 1# 1-前景注意图=背景注意图
...
# dual attention模块，解码器两个拼接后到这一个模块进行处理
...
class EAM(nn.Module):
    ...
    def forward(self, x4, x1):
        ...
        return out
# 计算给定查询（query）、键（key）和值（value）的注意力权重并应用这些权重来生成加权值
...
class CTO(nn.Module):
    ...
    def forward(self, x):
        ...
        trans = self.multi_trans(x1)#16,256,64,64后面是我的（1，256，64，64）
        
        s1 = run_sobel(self.sobel_x1, self.sobel_y1, x1)#（1，256，64，64）
        s4 = run_sobel(self.sobel_x4, self.sobel_y4, x4)#（1，2048，8，8）
       
        edge = self.eam(s4, s1)#（1，1，64，64）
        edge_att = torch.sigmoid(edge)#[16, 1, 64, 64]，后面是我的（1，1，64，64）
        # 通过插值使trans与x4的大小一致
        trans = F.interpolate(trans, x4.size()[2:], mode='bilinear', align_corners=False)#256,8,8后面（1，256，8，8）
        dual_attention = self.head(torch.cat([trans, x4], dim=1))[0]  #2048,8,8，后面（1，2048，8，8）
        
        x1a = x1*edge_att#（1，256，64，64）
        edge_att2 = F.interpolate(edge_att, x2.size()[2:], mode='bilinear', align_corners=False)#（1，1，32，32）
        x2a = x2*edge_att2#（1，512，32，32）
        edge_att3 = F.interpolate(edge_att, x3.size()[2:], mode='bilinear', align_corners=False)#（1，1，16，16）
        x3a = x3*edge_att3#（1，1024，16，16）

        
        x1r = self.reduce1(x1a) #（1，64，64，64）
        x2r = self.reduce2(x2a)#128,32,32后面（1，64，32，32）
        x3r = self.reduce3(x3a)#256,16,16后面（1，64，16，16）
        
        dual_attention = self.reduce4(dual_attention)#（1，64，8，8）
        ...
        o3 = self.predictor3(c3)#（1，2，16，16）
        o3 = F.interpolate(o3, scale_factor=16, mode='bilinear', align_corners=False)
        o2 = self.predictor2(c2)#（1，2，32，32）
        o2 = F.interpolate(o2, scale_factor=8, mode='bilinear', align_corners=False) 
        o1 = self.predictor1(c1)#（1，2，64，64）
        o1 = F.interpolate(o1, scale_factor=4, mode='bilinear', align_corners=False)#（1，2，256，256）
        oe = F.interpolate(edge_att, scale_factor=4, mode='bilinear', align_corners=False)#（1，1，256，256）

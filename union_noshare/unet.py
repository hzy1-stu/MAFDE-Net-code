...
class MICCANlong(nn.Module):
    ...
    def forward(self, x_under, xk_under, mask):
        # x_under是欠采样时域数据(相当于零填充图像)
        x_rec_dc = x_under
        for i in range(self.nlayer):  # range(1)
            # self.layer[i]就是第一个UNetCSE网络，将x_rec_dc传入该网络里进行前向传播
            x_rec = self.layer[i](x_rec_dc)  # x_rec就是从网络里出来的重建过的图像
            # 这里运用到了模型中的残差结构
            x_res = x_rec_dc + x_rec
            # 将相加过后的结果传入数据一致性DC层进行运算
            x_rec_dc = self.dc(mask, x_res, xk_under)

        # 先将输出结果转为复数，再用sos（平方和根）函数将数据转换成实数并将多通道转变为单通道（多通道磁共振就是这样，医生最后看的图片就是单通道的）
        recimg = x_rec_dc
        return recimg


def unet(in_channel, out_channel):

    model = MICCANlong(2, 2, 1)
    checkpoint = torch.load('/home/huangzhenyu/new_four/CHAOS/union_noshare/rec2DRUR3_model_best.pth.tar')['state_dict']
